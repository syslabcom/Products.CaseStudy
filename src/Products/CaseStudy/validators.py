...
class CaseStudyTextSizeValidator:
    """Tests if the sum of Abstract, Action and Result does exceed 350 words
    in english.
    """
    ...
    def __call__(self, value, *args, **kwargs):
        ...
        if self.words > 0 and len(words) > self.words:
            return ("Validation failed. Abstract, Action and Results have "
                    "%s words (%s total allowed)" % (len(words), self.words))

        if self.chars > 0 and len(total) > self.chars:
            return ("Validation failed. Abstract, Action and Results have "
                    "%s characters (%s total allowed)" % (
                        len(total), self.chars))
